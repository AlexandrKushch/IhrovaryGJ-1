...
public partial class Fork : Node3D
{
    ...
    private void HandleForkCollision()
    {
        ...
        if (TryRayToGround(closestItem, out var collisionPoint))
        {
    ...
    private void HandlePickUp()
    {
        ...
        if (Mathf.IsEqualApprox(closestItem.GlobalPosition.X, MovablePart.GlobalPosition.X, 0.5f)
            ...
            && TryRayToGround(closestItem, out var collisionPoint))
        {
    ...
    private void HandleDropDown()
    {
        ...
        if (Mathf.IsEqualApprox(MovablePart.Position.Y, MinY, 0.02f))
        {
    ...
    private bool TryRayToGround(Node3D from, out Vector3 position)
    {
        position = Vector3.Zero;

        var spaceState = GetWorld3D().DirectSpaceState;
        var query = PhysicsRayQueryParameters3D.Create(from.GlobalPosition, from.GlobalPosition + Vector3.Down);
